$(document).ready(function () {
    ...
    console.log("Load dropdown xong!")
});
    ...
    $.ajax({
        ...
        departments.forEach(department => {
            ...
            let dropdownItem = `<div class="dropdown-item" uuid="${departmentId}">
                                    <div class="dropdown-item-icon"><i class="fas fa-check"></i></div>
...
function applySelect(dropdown) {
    ...
    $(items).on('click', function (e, manuel) {
        if (typeof manuel === 'undefined' || manuel === false) {
            ...
            if (inputName === "place-dropdown" || inputName === "position-dropdown") {
                inputField.val(this.textContent.trim()).trigger("change");
...
function clearSelect(button) {
    ...
    if (inputOfDropdown && inputOfDropdown.length > 0) {
        $(inputOfDropdown[0]).val("").trigger("change");
    }
