	...
	$.fn.fancyTable = function (options) {
		...
		this.tableUpdate = function (elm) {
			...
			if (settings.pagination) {
				...
				// paginationElement.append('<a data-n="1" class="first-page btn-2" id="first-page"></a> <a data-n="1" class="prev-page btn-2" id="prev-page"></a>');
				for (var n = 1; n <= elm.fancyTable.pages; n++) {
					if (n == 1 || (n > (elm.fancyTable.page - (settings.pagClosest + 1)) && n < (elm.fancyTable.page + (settings.pagClosest + 1))) || n == elm.fancyTable.pages) {
						var a = $("<a>", {
							...
							// console.log(n);
							// console.log(elm.fancyTable.page);
							// $('.this-page-start-count').text((n - 1) * elm.fancyTable.perPage + 1);
							// $('.this-page-end-count').text((n - 1) * elm.fancyTable.perPage + elm.fancyTable.perPage);
							// $('.this-page-count').text(elm.fancyTable.matches);
							// $(paginationElement).parent().find("#prev-page").attr("data-n", n - 1);
				...
				// paginationElement.append(`<a data-n="2" class="next-page btn-2" id="next-page"></a> <a data-n="${elm.fancyTable.pages}" class="last-page btn-2" id="last-page"></a>`)
			}
